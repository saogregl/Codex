...
const DocumentCard: FC<DocumentCardProps> = ({ document, setSelectedObject, setOpen, open, tags, ...rest }) => {
    ...
    const options = {
        transform: (reactNode, domNode, index) => {
            ...
            if (domNode.name === "b") {
                if (theme == "g100") {
                    return <span key={index} className='data--highlight-text--g100'>{reactNode}</span>;
                } else {
                    return <span key={index} className='data--highlight-text'>{reactNode}</span>;
                }

            }
            else {
                return <span key={index} className='data--snippet-text'>{reactNode}</span>;
            }
        }
    }
    ...
    return (
        <div className={`${settings.sipePrefix}--card-content-wrapper`} {...rest}>
            <ProductiveCard
                ...
                pictogram={() => tags ? tags.map((tag, index) => (
                    <TagComponent key={index} type={tag.color.toLocaleLowerCase()}>{tag.name}</TagComponent>
                )) : null}
